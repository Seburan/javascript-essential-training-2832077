...
const newStrapLength = (strapArray) => {
  ...
  strapArray.forEach((listElement) => {
    ...
    lengthForm.innerHTML = `
      <input type="number" name="${side}Length" placeholder="New ${side} length" action="#">
    ...
    lengthForm.addEventListener("submit", function (event) {
      ...
      const formData = new FormData(lengthForm);

      // formdata gets modified by the formdata event
      // console.log(formData.get("leftLength")); // foo
      // console.log(formData.get("rightLength")); // bar

      const newLength = formData.get(`${side}Length`);
      console.log("Sumitted length : ", newLength);

      // find the element to update
      const lengthElement = listElement.querySelector("span");
